...
def fun(input: str):
    ...
    for line in input.splitlines():
        ...
        out : List[str] = pulls.split(";")
        wrong = False
        for round_ in out:
            round_ = round_.split(",")
            for val in round_:
                val = val.strip()
                number, col = val.split()
                if colors[col] < int(number):
                    colors[col] = int(number)
...
example2 = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
example = """Game 1: 12 red, 2 green, 5 blue; 9 red, 6 green, 4 blue; 10 red, 2 green, 5 blue; 8 blue, 9 red
