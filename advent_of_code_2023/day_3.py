...
def fun(input_: str):
    global colors
    res = 0
    splitlines = input_.splitlines()
    sum = 0
    for i, line in enumerate(splitlines):
        for j, _ in enumerate(splitlines[i]):
            char = splitlines[i][j]
            changes = []
            if char == '*':
                for y in [-1, 0, 1]:
                    for x in [-1, 0, 1]:
                        ...
                        splitlines_y = i + y
                        splitlines_x = j + x
                        if 0 > splitlines_x or splitlines_x >= len(splitlines[0]) or splitlines_y < 0 or splitlines_y >= len(splitlines):
                            continue
                        x_ = splitlines[splitlines_y][splitlines_x]
                        l, r = splitlines_x, splitlines_x
                        if x_.isnumeric():
                            while l - 1 >= 0 and splitlines[splitlines_y][l - 1].isnumeric():
                                l -= 1
                            while r + 1 < len(splitlines[0]) and splitlines[splitlines_y] \
                                    [r + 1].isnumeric():
                                r += 1
                            changes.append((int(splitlines[splitlines_y][l: r + 1]), splitlines_y, splitlines[splitlines_y]))
                            splitlines[splitlines_y] = splitlines[splitlines_y][: l] + ('.' * ((r + 1) - l)) + splitlines[splitlines_y][r + 1:]

            if len(changes) == 2:
                s = 1
                for val, _, _ in changes:
                    s *= val
                sum += s

            for _, line_n, l in changes:
                splitlines[line_n] = l
    return sum
...
example = """........................................................862...........20.............453...619......58........694...312.................292.
